...
def call_claude(system, user_msg, max_tok=800):
    if not ANTHROPIC_KEY: return "[AI غير مفعّل — أضف ANTHROPIC_API_KEY في Railway]"
    try:
        r = requests.post(ANTHROPIC,
            headers={"x-api-key":ANTHROPIC_KEY,"anthropic-version":"2023-06-01","Content-Type":"application/json"},
            json={"model":"claude-sonnet-4-20250514","max_tokens":max_tok,"system":system,
...
Commits: {tokenomics.get('commits',0)}/4أسابيع"""
    ...
    hawk = call_claude(HAWK_SYS, f"حلل السيولة:\n{pair_txt}\nالسوق: {mkt_txt}", 600)
    sage = call_claude(SAGE_SYS, f"حلل الأمان والتوكنوميكس:\n{sec_txt}\n{tok_txt}", 600)
    oracle = call_claude(ORACLE_SYS,
        f"الصقر:\n{hawk}\n\nالحكيم:\n{sage}\n\nالبيانات الكاملة:\n{pair_txt}{sec_txt}{tok_txt}\n{mkt_txt}\n\nأصدر الحكم النهائي للاستثمار 6-12 شهر.",
        900)
    conf = 50
    m = re.search(r'(\d{1,3})\s*(?:/100|٪|%)', oracle)
    if m: conf = min(100, int(m.group(1)))
    return {"hawk":hawk, "sage":sage, "oracle":oracle, "confidence":conf}
